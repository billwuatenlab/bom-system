...
async function migrate() {
    ...
    const bomRelations = sqlite.prepare('SELECT * FROM BomRelation').all();
    let bomCount = 0;
    ...
    const transactions = sqlite.prepare('SELECT * FROM InventoryTransaction').all();
    let txCount = 0;
    ...
    const logs = sqlite.prepare('SELECT * FROM OperationLog').all();
    let logCount = 0;
    for (const l of logs) {
        try {
    ...
    const metas = sqlite.prepare('SELECT * FROM CategoryMeta').all();
    let metaCount = 0;
